...
function checkServerResponse(responseBody: any, isCorrect: any) {
  assert(!responseBody.HTTPError, "Ошибка сервера");

  if (isCorrect) {
    describe("Магазин", function() {
        it('can be recived by id')
    })
...
describe("Магазин", function () {
  ...
  it("Успешное получение всех товаров", async () => {
    ...
    getResponseBody.forEach((product: any) => {
  ...
  it("Успешное создание товара", async () => {
    ...
    const getResponse = await productsController.getAllProducts();
    const getResponseBody = getResponse.body;
    let product: any = getResponseBody.find((item: any)=> item.id == createResponseBody.id);
  ...
  tests.forEach(({ args, expected }) => {
    it("Неуспешное создание товара с невалидными данными", async () => {
      ...
      let product = getResponseBody.find((item: any)=> item.id == createResponseBody.id);
  ...
  it("Неуспешное создание товара с пустыми данными", async () => {
    ...
    const getResponse = await productsController.getAllProducts();
    const getResponseBody = getResponse.body;
    const product = getResponseBody.find((item: any)=> item.id == createResponseBody.id);
  ...
  it("Успешное редактирование товара", async () => {
    ...
    let newProduct = getResponseBody.find((item: any)=> item.id == createResponseBody.id);
  ...
  it("Неуспешное редактирование несуществующего товара", async () => {
    ...
    const getResponse = await productsController.getAllProducts();
    const getResponseBody = getResponse.body;
    let product = getResponseBody.find((item: any)=> item.id == testData.nonexistentProduct.id);
  ...
  it('Генерация поля alias у создаваемых товаров', async () => {
    ...
    const firstProduct = getResponseBody.find((item: any)=> item.id == firstCreateResponseBody.id);
    const secondProduct = getResponseBody.find((item: any)=> item.id == secondCreateResponseBody.id);
    const thirdProduct = getResponseBody.find((item: any)=> item.id == thirdCreateResponseBody.id);
  ...
  it('Генерация поля alias у редактируемых товаров', async () => {
    ...
    let firstProduct = getResponseBody.find((item: any)=> item.id == firstCreateResponseBody.id);
    await productsController.editProduct({ ...firstProduct, ...testData.validProductForEdit });
    let secondProduct = getResponseBody.find((item: any)=> item.id == secondCreateResponseBody.id);
    await productsController.editProduct({ ...secondProduct, ...testData.validProductForEdit });
    let thirdProduct = getResponseBody.find((item: any)=> item.id == thirdCreateResponseBody.id);
    await productsController.editProduct({ ...thirdProduct, ...testData.validProductForEdit });

    getResponse = await productsController.getAllProducts();
    getResponseBody = getResponse.body;

    firstProduct = getResponseBody.find((item: any)=> item.id == firstCreateResponseBody.id);
    secondProduct = getResponseBody.find((item: any)=> item.id == secondCreateResponseBody.id);
    thirdProduct = getResponseBody.find((item: any)=> item.id == thirdCreateResponseBody.id);
